...
* **Admin模板(Admin Layout)**

Admin Layout 提供基于 Gin API、Vue 3 UI、JWT 认证和 Casbin RBAC 的后台管理系统模板。

* **MCP Server模板(MCP Server Layout)**

MCP Server Layout 提供支持 STDIO、SSE 和 Streamable HTTP 传输协议的 Model Context Protocol 服务模板。

* **Monorepo模板(Monorepo Layout)**

Monorepo Layout 适合在一个仓库中管理多个应用，包含共享包、Admin后端、内嵌管理前端和Home应用。
